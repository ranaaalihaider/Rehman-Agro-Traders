...
import { Plus, Trash2, Calendar, FileText, ShoppingCart, Loader2, FileCheck, Printer } from 'lucide-react';
...
const StockIn = () => {
  ...
  useEffect(() => {
    if (companyId) {
      ...
      setItemsList([{ itemId: '', itemName: '', quantity: 1, rate: 0, total: 0 }]);
    } else {
      setFilteredItems([]);
      setItemsList([]);
    }
  ...
  const handleAddItemRow = () => {
    setItemsList([
      ...itemsList,
      { itemId: '', itemName: '', quantity: 1, rate: 0, total: 0 },
    ]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        date,
        invoiceNumber,
        customerSupplierName: selectedComp ? selectedComp.companyName : '',
        notes,
      ...
      setError(err.response?.data?.message || 'Failed to submit Stock In invoice');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {successTx ? (
        <div className="space-y-6">
          <div className="glass-panel p-6 border border-emerald-200 bg-emerald-50/70 flex flex-col md:flex-row md:items-center justify-between gap-4 print-hide">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-slate-800 text-lg">Purchase Invoice Saved!</h3>
        ...
        /* Purchase stock entry form */
        <div className="space-y-6">
          <div>
            <h2 className="text-2xl font-bold text-slate-800 tracking-wide">Stock In (Purchase Entry)</h2>
            <p className="text-sm text-slate-500">Record incoming stock purchases from suppliers and update current stock.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Meta fields */}
            <div className="glass-panel p-5 border border-slate-200/50 bg-white grid gap-4 sm:grid-cols-2 md:grid-cols-3">
              <div>
              ...
              <div>
                ...
                >
                  <option value="" disabled>Select Supplier Company</option>
                  {companies.map((c) => (
              ...
              <div className="glass-panel p-5 border border-slate-200/50 bg-white space-y-4">
                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-slate-800 text-[15px]">Purchase Items Grid</h3>
                  </div>
                  <button
                    type="button"
                    onClick={handleAddItemRow}
                    className="btn-secondary py-1.5 px-3 text-xs bg-primary-50 text-primary-800 border-primary-200 hover:bg-primary-100"
                  >
                    <Plus size={14} />
                    Add Row
                  </button>
                ...
                <div className="pt-4 border-t border-slate-100 flex flex-col md:flex-row justify-between gap-6">
                  <div className="flex-1">
                    ...
                    <textarea
                      ...
                      placeholder="e.g. Received goods in warehouse, payment pending..."
                      value={notes}
                  ...
                  <div className="w-full md:w-64 glass-panel p-4 bg-slate-50/50 flex flex-col justify-center gap-2">
                    ...
                    <div className="flex justify-between border-t border-slate-200/60 pt-2 text-sm text-slate-800">
                      ...
                      <span className="font-sans font-extrabold text-primary-800 text-[16px]">
